...
public class IniciativaControlador {
    ...
    Sector sectorSeleccionado = panel.getSectorFiltroSeleccionado(); // Cambio aquí: obtiene un solo sector
    ...
    List<Iniciativa> resultado = iniciativasActuales.stream()
            ...
            .filter(ini -> filtrarPorSectorUnico(ini, sectorSeleccionado)) // Cambio aquí: usa filtro de sector único
    ...
    private boolean filtrarPorEstado(Iniciativa ini, String estado) {
     ...
     /**
     * Evalúa si una iniciativa pertenece al sector seleccionado.
     * Si el sector es null (representa "Todos"), no aplica restricción.
     *
     * @param ini Iniciativa a evaluar.
